...
# Sources lib/*.sh files - downloads from GitHub if running remotely
################################################################################

LIB_DIR="$SCRIPT_DIR/lib"
LIB_GITHUB_BASE="https://raw.githubusercontent.com/nmarxer/wsl-ubuntu-setup/main/lib"
LIB_FILES=("core.sh" "languages.sh" "containers.sh" "devtools.sh")

# If lib/ doesn't exist locally, download from GitHub (remote execution)
if [ ! -d "$LIB_DIR" ]; then
    LIB_DIR="$HOME/.wsl_ubuntu_setup_libs"
    mkdir -p "$LIB_DIR"
    echo "Downloading library modules from GitHub..."
    for lib_file in "${LIB_FILES[@]}"; do
        if [ ! -f "$LIB_DIR/$lib_file" ] || [ "$1" = "--force-download" ]; then
            curl -fsSL "$LIB_GITHUB_BASE/$lib_file" -o "$LIB_DIR/$lib_file" 2>/dev/null || {
                echo "ERROR: Failed to download $lib_file from GitHub"
                exit 1
            }
        fi
    done
